...
impl<'i> Value<'i> {
    pub fn parse_nom(input: &'i [u8], previous_tag: Option<&'i str>) -> IResult<&'i [u8], Self> {
        ...
        let (i, cap) = match id {
            Ok(id) => alt((
                map(preceded(tag(" bin"), bracketed(binary)), |data| {
                    Value::Binary { tag: id, data }
                }),
                map(bracketed(balancedparens), |value| Value::String {
                    tag: id,
                    value,
                }),
            ))(i),
...
fn ident(i: &[u8]) -> IResult<&[u8], &str> {
    map_res(
        take_while1(|c| is_alphanumeric(c) || c == b'_'),
        str::from_utf8,
    )(i)
}
